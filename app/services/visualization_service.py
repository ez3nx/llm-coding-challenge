        ...
        commit_data.append(
            {
                ...
                "total_changes": commit["stats"]["additions"]
...
def create_weekly_activity_chart(df_commits):
    ...
    # Настраиваем оси
    fig_day_of_week.update_xaxes(showgrid=False, zeroline=False)
...
def create_file_types_chart(commits):
    ...
    fig_extensions.update_traces(textposition="inside", textinfo="percent+label")
...
def display_commit_analytics(commits, author_data):
    ...
    with col1:
        st.metric("Total Commits", len(commits), delta=None)

    with col2:
        total_additions = df_commits["additions"].sum()
        st.metric("Lines Added", f"{total_additions:,}", delta=None)

    with col3:
        total_deletions = df_commits["deletions"].sum()
        st.metric("Lines Deleted", f"{total_deletions:,}", delta=None)

    with col4:
        total_files = df_commits["files_changed"].sum()
        st.metric("Files Changed", f"{total_files:,}", delta=None)
    ...
    with viz_tabs[1]:
        ...
        with col1:
            ...
            weekly_stats = (
                ...
                .agg({"additions": "sum", "deletions": "sum", "files_changed": "sum"})
                .reset_index()
            )
                ...
                for commit in commits:
                    for file in commit["files"]:
                        if isinstance(file, dict) and "filename" in file:
                            ...
                            changes = file.get("additions", 0) + file.get(
                                "deletions", 0
                            )
    ...
    recent_commits = df_commits.sort_values("date", ascending=False).head(10)

    # Создаем красивую таблицу коммитов с экспандерами для LLM анализа
    for i, row in recent_commits.iterrows():
        ...
        # ID для экспандера
        expander_id = f"commit_{commit_sha}"
        ...
        <div style="display: flex; padding: 12px; border-radius: 8px; background-color: {ALFA_GRAY}; margin-bottom: 8px; align-items: center;">
            ...
            <div style="flex: 1; font-weight: 500;">{has_llm}{commit_msg}</div>
        ...
        # Если есть LLM анализ, показываем его в экспандере
        if llm_summary:
